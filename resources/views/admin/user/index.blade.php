            ...
            <div class="bg-white dark:bg-slate-850 rounded-2xl shadow-xl p-4 mx-3">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-slate-600 dark:text-slate-400">
                        Hiển thị <span class="font-semibold text-slate-800 dark:text-white">{{ $viewData['users']->firstItem() }}</span> 
                        đến <span class="font-semibold text-slate-800 dark:text-white">{{ $viewData['users']->lastItem() }}</span> 
                        trong tổng số <span class="font-semibold text-slate-800 dark:text-white">{{ $viewData['users']->total() }}</span> người dùng
                    </div>
                    
                    <div class="flex justify-center gap-1">
                        <a href="{{ $viewData['users']->url(1) }}" class="inline-flex items-center justify-center w-10 h-10 leading-normal text-center transition-all ease-in rounded-lg shadow-md hover:shadow-lg {{ $viewData['users']->onFirstPage() ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-slate-50 text-slate-700' }}">
                            <i class="fa-solid fa-angles-left"></i>
                        </a>
                        <a href="{{ $viewData['users']->previousPageUrl() }}" class="inline-flex items-center justify-center w-10 h-10 leading-normal text-center transition-all ease-in rounded-lg shadow-md hover:shadow-lg {{ $viewData['users']->onFirstPage() ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-slate-50 text-slate-700' }}">
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            <a href="{{ $viewData['users']->url($i) }}" class="inline-flex items-center justify-center w-10 h-10 text-sm font-semibold leading-normal text-center transition-all ease-in rounded-lg shadow-md hover:shadow-lg {{ $i == $viewData['users']->currentPage() ? 'bg-blue-500 text-white' : 'bg-white hover:bg-slate-50 text-slate-700' }}">{{ $i }}</a>
                        @endfor

                        <a href="{{ $viewData['users']->nextPageUrl() }}" class="inline-flex items-center justify-center w-10 h-10 leading-normal text-center transition-all ease-in rounded-lg shadow-md hover:shadow-lg {{ !$viewData['users']->hasMorePages() ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-slate-50 text-slate-700' }}">
                            <i class="fa-solid fa-angle-right"></i>
                        </a>
                        <a href="{{ $viewData['users']->url($last) }}" class="inline-flex items-center justify-center w-10 h-10 leading-normal text-center transition-all ease-in rounded-lg shadow-md hover:shadow-lg {{ !$viewData['users']->hasMorePages() ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-white hover:bg-slate-50 text-slate-700' }}">
