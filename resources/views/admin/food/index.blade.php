        ...
        <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 wrap-break-word bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                ...
                <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-center">
                    ...
                    <a href="{{ route('food.new') }}" class="inline-block px-2 py-2 mb-2 text-xs font-bold leading-normal text-center text-white capitalize transition-all ease-in rounded-lg shadow-md bg-success bg-150 hover:shadow-xs"><i class="fa-solid fa-plus mr-274"></i> Thêm món mới</a>
                </div>
            ...
            <div class=" flex justify-center px-2 gap-1">
                ...
                <a href="{{ $viewData['foods']->url(1) }}" class="inline-block px-3 py-2 leading-normal text-center transition-all ease-in rounded-lg shadow-md fw-bolder bg-150 hover:shadow-xs {{ $viewData['foods']->onFirstPage() ? 'pointer-events-none opacity-50 cursor-not-allowed' : '' }}">
                    <i class="fa-solid fa-angles-left"></i>
                </a>
                <a href="{{ $viewData['foods']->previousPageUrl() }}" class="inline-block px-3 py-2 leading-normal text-center transition-all ease-in rounded-lg shadow-md fw-bolder bg-150 hover:shadow-xs {{ $viewData['foods']->onFirstPage() ? 'pointer-events-none opacity-50 cursor-not-allowed' : '' }}">
                ...
                @for($i = $start; $i <= $end; $i++)
                    <a href="{{ $viewData['foods']->url($i) }}" class="inline-block px-3 py-2 leading-normal text-center {{ $i == $viewData['foods']->currentPage() ? 'bg-blue-500 text-white' : 'text-blue-500' }} transition-all ease-in rounded-lg shadow-md fw-bolder bg-150 hover:shadow-xs">{{ $i  }}</a>
                @endfor

                <a href="{{ $viewData['foods']->nextPageUrl() }}" class="inline-block px-3 py-2 leading-normal text-center transition-all ease-in rounded-lg shadow-md fw-bolder bg-150 hover:shadow-xs {{ !$viewData['foods']->hasMorePages() ? 'pointer-events-none opacity-50 cursor-not-allowed' : '' }}">
                    <i class="fa-solid fa-angle-right"></i>
                </a>
                <a href="{{ $viewData['foods']->url($last) }}" class="inline-block px-3 py-2 leading-normal text-center transition-all ease-in rounded-lg shadow-md fw-bolder bg-150 hover:shadow-xs {{ !$viewData['foods']->hasMorePages() ? 'pointer-events-none opacity-50 cursor-not-allowed' : '' }}">
