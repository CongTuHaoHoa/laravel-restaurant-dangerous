...
@section('content')
    <form action="{{ route('food.index') }}" method="POST" enctype="multipart/form-data">
        @csrf
                        ...
                        <div class="flex flex-wrap -mx-3">
                            <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                <div class="mb-4">
                                    ...
                                    <input id="FOD_NAME" type="text" name="FOD_NAME" placeholder="Nhập tên món ăn" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" />
                                </div>
                            </div>
                            <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                <div class="mb-4">
                                    <label for="FOD_PRICE" class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700 dark:text-white/80">Giá tiền</label>
                                    <input id="FOD_PRICE" type="number" name="FOD_PRICE" placeholder="Nhập giá tiền" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" />
                                </div>
                            </div>
                            <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                    ...
                                    <div class="px-2 py-2 rounded-1.8 border-b-slate-50 border">
                                        @foreach(@$viewData['categories'] as $category)
                                            <div class="form-check flex align-items-center">
                                                <input class="form-check-input mr-2" type="checkbox" id="{{ $category->CTG_ID }}" name="{{ $category->CTG_ID }}">
                            ...
                            <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                <div class="mb-4">
                                    <div class="form-floating">
                                        <textarea class="form-control" placeholder="Leave a comment here" id="FOD_DESCRIPTION" name="FOD_DESCRIPTION" style="height: 100px"></textarea>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div id="drop-zone" class="relative group">
                                        <div class="relative overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-850 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl transition-all duration-300 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-lg" style="min-height: 320px;">
                                            <!-- Preview Image -->
                                            <img id="food-image-previewer" class="hidden absolute inset-0 w-full h-full object-cover rounded-2xl z-10" src="" alt="Preview"/>
                                            ...
                                            <!-- Change Image Button (Appears on Hover when image exists) -->
